...
public class BitmapUtils {
     ...
     * @param foreBitmap 盖在上面的位图
     * @return
     */
    public static Bitmap mergeBitmap(Bitmap backBitmap, Bitmap foreBitmap) {

        if (backBitmap == null || backBitmap.isRecycled()
                || foreBitmap == null || foreBitmap.isRecycled()) {
            Log.e(TAG, "backBitmap=" + backBitmap + ";frontBitmap=" + foreBitmap);
            return null;
        }
        Bitmap bitmap = backBitmap.copy(Bitmap.Config.ARGB_8888, true);
        Canvas canvas = new Canvas(bitmap);
        Rect baseRect = new Rect(0, 0, backBitmap.getWidth(), backBitmap.getHeight());
        Rect frontRect = new Rect(0, 0, foreBitmap.getWidth(), foreBitmap.getHeight());
        canvas.drawBitmap(foreBitmap, frontRect, baseRect, null);
        return bitmap;
    }
